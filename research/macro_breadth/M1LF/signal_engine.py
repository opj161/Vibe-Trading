...
class SignalEngine:
    def generate(self, data_map: Dict[str, pd.DataFrame]) -> Dict[str, pd.Series]:
        ...
        # M1LF deployment-expression variant (CPD-1 go-live decision support,
        # option (c) "hold flat on macro short signals"): reproduce EXACTLY what
        # live deployment does when short tickets are skipped -- the live
        # signal's gross-exposure clip (base.py::_align / normalize_gross_
        # exposure) normalizes with the short legs' |weight| still included,
        # and only THEN are the short tickets skipped. So: normalize the full
        # signed book first, then zero the negatives. (Clipping before
        # normalizing would overstate the long legs' size vs. real deployment.)
        # The outer _align clip then sees gross <= 1 and is a no-op.
        gross = result.abs().sum(axis=1).clip(lower=1.0)
        result = result.div(gross, axis=0).clip(lower=0.0)
